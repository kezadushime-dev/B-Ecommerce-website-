...
export const PromoGrid: React.FC = () => {
  return (
    <div className="w-full lg:w-2/5 p-4 relative group bg-white">
      ...
      <button title="Previous" className="p-prev absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white border p-1 opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft size={20}/></button>
      <button title="Next" className="p-next absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white border p-1 opacity-0 group-hover:opacity-100 transition-opacity"><ChevronRight size={20}/></button>
    </div>
  );
};